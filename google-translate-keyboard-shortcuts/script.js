...
document.addEventListener('keydown', (event) => {
    ...
    function clickSourceLanguage(languageCode) {
        click(`[role=listbox]:has([data-language-code=auto]) [role=option][data-language-code=${languageCode}][aria-selected=false]`);
    }
    function clickDestinationLanguage(languageCode) {
        click(`[role=listbox]:not(:has([data-language-code=auto])) [role=option][data-language-code=${languageCode}][aria-selected=false]`);
    }
    if (altKey) {
        switch (code) {
            case 'KeyS':
